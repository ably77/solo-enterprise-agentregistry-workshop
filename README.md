...
- Cataloged MCP servers of three shapes (local `stdio`, public remote, and self-hosted in-cluster), including ones that need their own API key, kept in a k8s `Secret` and out of the catalog
...
The AWS Bedrock AgentCore labs are opt-in because they create real, billable AWS resources:
